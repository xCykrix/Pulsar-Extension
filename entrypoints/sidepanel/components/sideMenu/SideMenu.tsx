...
// import type { AccessCache, UserAccess } from '../../../shared/AccessCache.ts';
// import type { UseAuthentication } from '../../hooks/useAuthentication.ts';
// import { KeywordInput } from '../keyword/KeywordInput.tsx';
...
//   const [isSubmittingCreateGroup, setIsSubmittingCreateGroup] = useState(false);
...
//   const hasSessionToken = useAuthentication.sessionToken !== null && useAuthentication.sessionToken.trim().length > 0;
//   const hasPartialTimeRange = (startTime === null && endTime !== null) || (startTime !== null && endTime === null);
//   const canSubmitCreateGroup = hasSessionToken
//     && guildId !== ''
//     && !isInflightValidating
//     && !isSubmittingCreateGroup
//     && genericValidationReject === 'OK'
//     && guildValidationReject === 'OK'
//     && groupNameValidationReject === 'OK'
//     && groupName.trim() !== ''
//     && !hasPartialTimeRange
//     && isTimeValid(startTime)
//     && isTimeValid(endTime)
//     && keywordsValidationReject === 'OK'
//     && !isKeywordCountExceeded;
...
//     if (!canSubmitCreateGroup || useAuthentication.sessionToken === null) {
//       return;
//     }
...
//       payload.activeTimeStart = formatNotificationTime(startTime, startMeridiem);
//       payload.activeTimeEnd = formatNotificationTime(endTime, endMeridiem);
//     }
//     else {
//       payload.activeTimeStart = null;
//       payload.activeTimeEnd = null;
//     }

//     setCreateGroupError(null);
//     setIsSubmittingCreateGroup(true);

//     try {
//       const response = await fetch(getEndpoint('/ui/createUserGroup'), {
//         method: 'POST',
//         headers: {
//           'Authorization': `Bearer ${useAuthentication.sessionToken}`,
...
//       if (!response.ok) {
//         setCreateGroupError(await getCreateGroupErrorMessage(response));
//         return;
//       }
...
//     }
//     catch (error) {
//       console.error('[CreateGroup] Create request failed.', error);
//       setCreateGroupError('Could not create the notification group. Please try again.');
//     }
//     finally {
//       setIsSubmittingCreateGroup(false);
//     }
//   };
...
//             />
//             <div className='h-2' aria-hidden='true' />
//             {((guildId !== '' && genericValidationReject !== 'OK') || createGroupError !== null) && (
//               <p className='text-center text-sm text-error'>
//                 {createGroupError ?? 'Could not validate the group right now. Please try again.'}
//               </p>
//             )}
//           </div>
//           <div className='flex justify-end gap-2'>
//             <button className='btn btn-ghost' type='button' disabled={isSubmittingCreateGroup} onClick={closeCreateGroupModal}>Cancel</button>
//             <button className='btn btn-primary' type='button' disabled={!canSubmitCreateGroup} onClick={() => void handleCreateGroup()}>
//               {isSubmittingCreateGroup && <span className='loading loading-spinner loading-xs' aria-hidden='true' />}
//               {isSubmittingCreateGroup ? 'Creating...' : 'Create'}
//             </button>
//           </div>
//         </div>
//         <label className='modal-backdrop' onClick={closeCreateGroupModal} />
//       </div>
//     </>
//   );
// }

// async function getCreateGroupErrorMessage(response: Response): Promise<string> {
//   try {
//     const errorBody = await response.json() as Record<string, unknown>;
//     if (typeof errorBody.cause === 'string' && errorBody.cause.trim().length > 0) {
//       return errorBody.cause;
//     }
...
//   }
//   catch {
//     // Ignore JSON parsing issues and fall back to the HTTP status below.
//   }

//   return `Could not create the notification group (${response.status} ${response.statusText}).`;
// }

// function formatNotificationTime(value: string, meridiem: 'AM' | 'PM'): `${number}:${number} ${'AM' | 'PM'}` {
//   return `${value} ${meridiem}` as `${number}:${number} ${'AM' | 'PM'}`;
// }
